...
class FlexibleArchProblem:
    ...
    def __init__(self):
        ...
        self.max_alloc = 4
        self.src_actor_for_latency = "a"  # for latency
        self.dst_actor_for_latency = "c"  # for latency
...
def run_sdf3(xml_file, src_actor="a", dst_actor="d"):  # for latency
    # First get throughput
    cmd_thr = [  # for latency
    ...
    # Then get latency  # for latency
    cmd_lat = [  # for latency
        "/mnt/d/SDF3/sdf3/build/release/Linux/bin/sdf3analysis-sdf",
        "--graph", xml_file,
        "--algo", f"latency(min,{src_actor},{dst_actor})"
    ]  # for latency

    env = os.environ.copy()
    env["LD_LIBRARY_PATH"] = "/mnt/d/SDF3/sdf3/build/release/Linux/lib:" + env.get("LD_LIBRARY_PATH", "")

    throughput = 0.0  # for latency
    latency = float('inf')  # for latency

    try:
        output_thr = subprocess.check_output(cmd_thr).decode()  # for latency
        throughput = parse_throughput(output_thr)  # for latency
    except:  # for latency
        throughput = 0.0  # for latency

    try:  # for latency
        output_lat = subprocess.check_output(cmd_lat).decode()  # for latency
        latency = parse_latency(output_lat)  # for latency
    except:  # for latency
        latency = float('inf')  # for latency

    return throughput, latency  # for latency
...
def parse_latency(output):  # for latency
    for line in output.splitlines():  # for latency
        line = line.strip()  # for latency
        if line.startswith("latency("):  # for latency
            val = line.split("=")[-1].strip()  # for latency
            if val == "inf":  # for latency
                return 1e6  # for latency
            return float(val)  # for latency
    return float('inf')  # for latency
...
creator.create("FitnessMulti", base.Fitness,
               weights=(1.0, -1.0, -1.0))  # for latency (max throughput, min cost, min latency)
...
def evaluate(individual):
    ...
    throughput, latency = run_sdf3(full_xml_path, problem.src_actor_for_latency,
                                   problem.dst_actor_for_latency)  # for latency
    ...
    return throughput, cost, latency  # for latency
...
def save_evolution_plot(population, generation, save_dir="evolution_plots"):
    ...
    latencies = [ind.fitness.values[2] for ind in valid_individuals]  # for latency
    ...
    nd_latencies = [ind.fitness.values[2] for ind in non_dominated]  # for latency

    # Create two subplots: Throughput vs Cost and Throughput vs Latency  # for latency
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(15, 6))  # for latency

    # Plot 1: Throughput vs Cost  # for latency
    ax1.scatter(throughputs, costs, alpha=0.6, c='blue', s=50,
                label=f'Population (n={len(valid_individuals)})', edgecolors='black', linewidth=0.5)  # for latency

    if nd_throughputs:  # for latency
        sorted_pairs_cost = sorted(zip(nd_throughputs, nd_costs))  # for latency
        if sorted_pairs_cost:  # for latency
            nd_thr_sorted_cost, nd_cost_sorted = zip(*sorted_pairs_cost)  # for latency
            ax1.scatter(nd_thr_sorted_cost, nd_cost_sorted, c='red', s=150,  # for latency
                        marker='*', label=f'Pareto Front (n={len(nd_throughputs)})',  # for latency
                        edgecolors='darkred', linewidth=1.5, zorder=5)  # for latency
            ax1.plot(nd_thr_sorted_cost, nd_cost_sorted, 'r--', alpha=0.5, linewidth=1)  # for latency

    ax1.set_xlabel("Throughput", fontsize=12, fontweight='bold')  # for latency
    ax1.set_ylabel("Cost", fontsize=12, fontweight='bold')  # for latency
    ax1.set_title(f"Generation {generation} - Throughput vs Cost", fontsize=12, fontweight='bold')  # for latency
    ax1.legend(loc='best', framealpha=0.9, fontsize=10)  # for latency
    ax1.grid(True, alpha=0.3, linestyle='--')  # for latency

    # Plot 2: Throughput vs Latency  # for latency
    ax2.scatter(throughputs, latencies, alpha=0.6, c='green', s=50,  # for latency
                label=f'Population (n={len(valid_individuals)})', edgecolors='black', linewidth=0.5)  # for latency

    if nd_throughputs:  # for latency
        sorted_pairs_lat = sorted(zip(nd_throughputs, nd_latencies))  # for latency
        if sorted_pairs_lat:  # for latency
            nd_thr_sorted_lat, nd_lat_sorted = zip(*sorted_pairs_lat)  # for latency
            ax2.scatter(nd_thr_sorted_lat, nd_lat_sorted, c='red', s=150,  # for latency
                        marker='*', label=f'Pareto Front (n={len(nd_throughputs)})',  # for latency
                        edgecolors='darkred', linewidth=1.5, zorder=5)  # for latency
            ax2.plot(nd_thr_sorted_lat, nd_lat_sorted, 'r--', alpha=0.5, linewidth=1)  # for latency

    ax2.set_xlabel("Throughput", fontsize=12, fontweight='bold')  # for latency
    ax2.set_ylabel("Latency", fontsize=12, fontweight='bold')  # for latency
    ax2.set_title(f"Generation {generation} - Throughput vs Latency", fontsize=12, fontweight='bold')  # for latency
    ax2.legend(loc='best', framealpha=0.9, fontsize=10)  # for latency
    ax2.grid(True, alpha=0.3, linestyle='--')  # for latency

    info_text = f"Best Throughput: {max(throughputs):.4f}\nBest Cost: {min(costs):.2f}\nBest Latency: {min(latencies):.4f}"  # for latency
    fig.text(0.02, 0.98, info_text, transform=fig.transFigure,  # for latency
             fontsize=9, verticalalignment='top',
             bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.5))  # for latency
...
def save_generation_data(population, generation, save_dir="evolution_data"):
    """
    Save throughput, cost, and latency of all individuals in a generation to CSV file
    """
    ...
    for idx, ind in enumerate(valid_individuals):
        ...
        latency = ind.fitness.values[2]  # for latency
        ...
        data.append({
            ...
            'cost': cost,
            'latency': latency,  # for latency
            'allocation': str(list(alloc)),
            'binding': str(list(binding)),
    ...
    with open(filename, 'w', newline='') as f:
        writer = csv.DictWriter(f, fieldnames=['individual_id', 'throughput', 'cost', 'latency',  # for latency
    ...
    best_latency = min(data, key=lambda x: x['latency'])  # for latency
    ...
    with open(summary_file, 'a', newline='') as f:
        ...
        if not file_exists:
            writer.writerow(
                ['generation', 'best_throughput', 'best_throughput_cost', 'best_throughput_latency',  # for latency
                 'best_cost', 'best_cost_throughput', 'best_cost_latency',  # for latency
                 'best_latency', 'best_latency_throughput', 'best_latency_cost', 'population_size'])  # for latency
        writer.writerow([generation,
                         best_throughput['throughput'], best_throughput['cost'], best_throughput['latency'],
                         # for latency
                         best_cost['cost'], best_cost['throughput'], best_cost['latency'],  # for latency
                         best_latency['latency'], best_latency['throughput'], best_latency['cost'],  # for latency
                         len(valid_individuals)])  # for latency
...
def save_all_generations_data(all_generations_data, save_dir="evolution_data"):
    ...
    with open(filename, 'w', newline='') as f:
        writer = csv.DictWriter(f, fieldnames=['generation', 'individual_id', 'throughput',
                                               'cost', 'latency', 'allocation', 'binding',
                                               'total_pe_count'])  # for latency
        writer.writeheader()
...
def plot_throughput_cost_evolution(all_generations_data, save_dir="evolution_data"):
    """
    Create plots showing evolution of throughput, cost, and latency across generations
    """
    os.makedirs(save_dir, exist_ok=True)
    ...
    best_throughputs = []
    best_costs = []
    best_latencies = []  # for latency
    avg_throughputs = []
    avg_costs = []
    avg_latencies = []  # for latency
    ...
    for gen_idx, gen_data in enumerate(all_generations_data):
        if gen_data:
            ...
            best_latencies.append(min(d['latency'] for d in gen_data))  # for latency
            avg_throughputs.append(np.mean([d['throughput'] for d in gen_data]))
            avg_costs.append(np.mean([d['cost'] for d in gen_data]))
            avg_latencies.append(np.mean([d['latency'] for d in gen_data]))  # for latency

    if not generations:
        return

    # Create three subplots  # for latency
    fig, axes = plt.subplots(1, 3, figsize=(18, 5))  # for latency

    # Plot 1: Throughput evolution  # for latency
    axes[0].plot(generations, best_throughputs, 'b-', label='Best Throughput', linewidth=2, marker='o')  # for latency
    axes[0].plot(generations, avg_throughputs, 'b--', label='Average Throughput', linewidth=1, alpha=0.7)  # for latency
    axes[0].set_xlabel('Generation', fontsize=12)  # for latency
    axes[0].set_ylabel('Throughput', fontsize=12)  # for latency
    axes[0].set_title('Throughput Evolution Across Generations', fontsize=14)  # for latency
    axes[0].legend()  # for latency
    axes[0].grid(True, alpha=0.3)  # for latency

    # Plot 2: Cost evolution  # for latency
    axes[1].plot(generations, best_costs, 'r-', label='Best Cost', linewidth=2, marker='o')  # for latency
    axes[1].plot(generations, avg_costs, 'r--', label='Average Cost', linewidth=1, alpha=0.7)  # for latency
    axes[1].set_xlabel('Generation', fontsize=12)  # for latency
    axes[1].set_ylabel('Cost', fontsize=12)  # for latency
    axes[1].set_title('Cost Evolution Across Generations', fontsize=14)  # for latency
    axes[1].legend()  # for latency
    axes[1].grid(True, alpha=0.3)  # for latency

    # Plot 3: Latency evolution  # for latency
    axes[2].plot(generations, best_latencies, 'g-', label='Best Latency', linewidth=2, marker='o')  # for latency
    axes[2].plot(generations, avg_latencies, 'g--', label='Average Latency', linewidth=1, alpha=0.7)  # for latency
    axes[2].set_xlabel('Generation', fontsize=12)  # for latency
    axes[2].set_ylabel('Latency', fontsize=12)  # for latency
    axes[2].set_title('Latency Evolution Across Generations', fontsize=14)  # for latency
    axes[2].legend()  # for latency
    axes[2].grid(True, alpha=0.3)  # for latency
...
def save_pareto_front_data(front, generation, save_dir="evolution_data"):
    ...
    with open(filename, 'w', newline='') as f:
        writer = csv.DictWriter(f, fieldnames=['pareto_index', 'throughput', 'cost', 'latency',  # for latency
                                               'allocation', 'binding', 'total_pe_count'])
        writer.writeheader()
        writer.writerows(data)
...
def main():
    ...
    for gen in range(1, NGEN + 1):
        ...
        # operator new - save generation data        gen_data = save_generation_data(pop, gen)
        all_generations_data.append(gen_data)
        ...
        best_latency = min(ind.fitness.values[2] for ind in pop if ind.fitness.valid)  # for latency
        print(
            f"Generation {gen}: Best Throughput={best_throughput:.4f}, Best Cost={best_cost:.2f}, Best Latency={best_latency:.4f}")  # for latency
    ...
    for i, ind in enumerate(sorted(front, key=lambda x: x.fitness.values[0], reverse=True)):
        print(
            f"{i + 1}. Throughput: {ind.fitness.values[0]:.6f} | Cost: {ind.fitness.values[1]:.2f} | Latency: {ind.fitness.values[2]:.4f}")  # for latency
