...
class FlexibleArchProblem:
    ...
    def __init__(self):
        ...
        self.actors_sil = [3, 2, 2, 3] # safety
...
# ==============================
# Safety Pattern Management Module
# ==============================

class SafetyManagement:
    """
    Manages the application of safety patterns to individuals based on 
    Safety Integrity Levels (SIL) of each task.
    
    This class handles pattern recognition, pattern application, and 
    provides a clean interface for safety-related operations.
    """
    
    def __init__(self, problem):
        """
        Initialize the Safety Management with a reference to the problem.
        
        Args:
            problem: Instance of FlexibleArchProblem containing task data
                    and SIL levels
        """
        self.problem = problem
        self.pattern_mapping = self._initialize_pattern_mapping()
    
    def _initialize_pattern_mapping(self):
        """
        Initialize the mapping between SIL levels and corresponding patterns.
        
        Returns:
            dict: Mapping of SIL levels to pattern names
        """
        # SIL 1: No pattern needed (basic system)
        # SIL 2: Homogeneous Duplex Pattern (HmD)
        # SIL 3: Heterogeneous Duplex Pattern (HtD)
        # SIL 4: More complex patterns (future extension)
        return {
            1: "NO_PATTERN",
            2: "HMD_PATTERN",  # Homogeneous Duplex
            3: "HTD_PATTERN",  # Heterogeneous Duplex
            4: "COMPLEX_PATTERN"  # For future extension
        }
    
    def recog_pattern(self, sil_level):
        """
        Recognize the appropriate safety pattern based on SIL level.
        
        Args:
            sil_level (int): Safety Integrity Level (1-4)
            
        Returns:
            str: Name of the pattern to apply
        """
        # TODO: Implement pattern recognition logic
        pass
    
    def HmD_pattern(self, individual, task_idx):
        """
        Apply Homogeneous Duplex Pattern (HmD).
        
        This pattern creates two identical channels (modules) to handle
        random hardware faults. It consists of a primary (active) module
        and a secondary (standby) module.
        
        According to the reference book (Armoush, 2010):
        - Suitable for SIL 2 systems
        - Handles random faults only
        - Does not handle systematic faults
        
        Args:
            individual (list): The chromosome [allocations, bindings]
            task_idx (int): Index of the task being processed
            
        Returns:
            list: Modified individual with HmD pattern applied
        """
        # TODO: Implement Homogeneous Duplex Pattern logic
        # - Duplicate the same processor type for the task
        # - Ensure at least 2 identical processors exist
        # - Update binding indices accordingly
        pass
    
    def HtD_pattern(self, individual, task_idx):
        """
        Apply Heterogeneous Duplex Pattern (HtD).
        
        This pattern creates two different (diverse) channels to handle
        both random and systematic hardware faults. The two modules
        are independently designed and implemented.
        
        According to the reference book (Armoush, 2010):
        - Suitable for SIL 3 systems
        - Handles both random and systematic faults
        - Higher development cost than HmD
        ...
        Returns:
            list: Modified individual with HtD pattern applied
        """
        # TODO: Implement Heterogeneous Duplex Pattern logic
        # - Ensure at least 2 different processor types exist
        # - One primary and one backup (diverse) processor
        # - Update binding indices accordingly
        pass
    

    def apply_safety_patterns(self, individual):
        """
        Apply safety patterns to all tasks in the individual.
        
        This method iterates through all tasks, determines their SIL levels,
        and applies the appropriate safety patterns.
        
        Args:
            individual (list): The chromosome [allocations, bindings]
            
        Returns:
            list: Modified individual with all safety patterns applied
        """
        # TODO: Implement full pattern application
        # - Iterate through all tasks
        # - Get SIL level for each task
        # - Call applying_pattern for each task
        # - Return the fully modified individual
        pass
    
    def get_sil_level(self, task_name):
        """
        Get the SIL level for a specific task.
        ...
        Returns:
            int: SIL level (1-4)
        """
        # TODO: Implement SIL retrieval logic
        # - Get task index from problem.tasks
        # - Return corresponding SIL from problem.sil_levels
        pass
    
    def update_sil_level(self, task_name, new_sil):
        """
        Update the SIL level for a specific task.
        
        This allows dynamic adjustment of SIL levels during runtime.
        
        Args:
            task_name (str): Name of the task
            new_sil (int): New SIL level (1-4)
            
        Returns:
            bool: True if update successful, False otherwise
        """
        # TODO: Implement SIL update logic
        # - Find task index
        # - Update problem.sil_levels
